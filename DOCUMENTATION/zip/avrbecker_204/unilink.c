...
void unilink_myid_cmd(void)
{
	switch(unilink_rxdata[cmd1]) 													// Switch on CMD1
  {
		...
		unsigned char msg[4] = STATUS_MSG;
                unilink_tx(msg);
		if (unilink_status == 0x40) { //changing
                  unilink_status = 0x20; //changed
                }
                else {
                  if (unilink_status == 0x20) { 
                    unilink_status = 0x21; //seeking
                  } 
                  else {
                    if (unilink_status == 0x21) {
                      unilink_status = 0x00; //playing 
                    }
                  }
                }
...
unilink_status = 0x40;
//tkn
	//				unsigned char msg75[13] = SEEK_MSG;					// send seek to disc and track
	//				unilink_tx(msg75);

//					if (unilink_status == 0x21) {
//                                          unilink_status = 0x00; }
//                                        else {unilink_status = 0x21;}                        // set PLAY status
//					unsigned char msg7[13] = PONG_CHANGING;					// send seek to disc and track
//					unilink_tx(msg7);
//					unsigned char msg77[10] = NOINFO;					// send seek to disc and track
//					unilink_tx(msg77);
//					unilink_status = 0x80;                        // set PLAY status
//					unilink_tx(msg7);
//tkn			    
	break;
    }; // end switch CMD 1
...
//                if (tracknbr>1) tracknbr=0;
  ...
  if (unilink_command.mode)									// send disc info
	{
		unsigned char msg773[8] = MODE_MSG;
    unilink_tx(msg773);
		unilink_command.mode = false;
		return;
	}
